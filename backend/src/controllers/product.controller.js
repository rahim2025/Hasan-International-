...
export const createProduct = async (req, res) => {
    try {
        const { name, description, inStock, contactPhone } = req.body;
...
export const updateProduct = async (req, res) => {
    try {
        ...
        if (contactPhone !== undefined) product.contactPhone = contactPhone;
        ...
        if (req.body.price) {
            ...
            // Store original price data from request
            const originalPriceData = { ...priceData };
            
            // Set defaults from existing product price if not provided
            priceData = {
                currency: originalPriceData.currency || product.price.currency || "USD",
                unit: originalPriceData.unit || product.price.unit || "per item",
                contactForPrice: originalPriceData.contactForPrice !== undefined ? originalPriceData.contactForPrice : false
            };

            // Validate and add price values
            if (!priceData.contactForPrice) {
                // For regular pricing, we need at least minValue
                if (!originalPriceData.minValue && originalPriceData.minValue !== 0) {
                    return res.status(400).json({
                        message: "Price is required when not using 'Contact for pricing'"
                    });
                }
                
                priceData.minValue = Number(originalPriceData.minValue);
                
                // Add maxValue if provided
                if (originalPriceData.maxValue !== undefined && originalPriceData.maxValue !== null && originalPriceData.maxValue !== "") {
                    priceData.maxValue = Number(originalPriceData.maxValue);
